...
def main(args):
    ...
    tokenizer1.save_pretrained(args.model_filename1)
    
    tokenizer2 = AutoTokenizer.from_pretrained(args.tokenizer2)
    tokenizer2.save_pretrained(args.model_filename2)

    
    LOGGER.info("Preprocess datasets...")
    train_size  = int(args.train_ratio * len(df))
    val_size    = int(args.valid_ratio * len(df))
    ...
    input_ids_train1, attention_masks_train1, labels_train1 = encode(train_df, tokenizer1)
    input_ids_train2, attention_masks_train2, labels_train2 = encode(train_df, tokenizer2)
    input_ids_val1, attention_masks_val1, labels_val1 = encode(val_df, tokenizer1)
    input_ids_val2, attention_masks_val2, labels_val2 = encode(val_df, tokenizer2)

    LOGGER.info(f"Labels: {labels_train1}")

    train_dataset1 = TensorDataset(input_ids_train1, attention_masks_train1, labels_train1)
    valid_dataset1 = TensorDataset(input_ids_val1, attention_masks_val1, labels_val1)
    
    train_dataset2 = TensorDataset(input_ids_train2, attention_masks_train2, labels_train2)
    valid_dataset2 = TensorDataset(input_ids_val2, attention_masks_val2, labels_val2)
    ...
    config = transformers.BertConfig.from_pretrained(args.base_model2, num_labels=1)
    ...
    num_training_steps = len(train_loader1) * args.n_epochs
    ...
    for epoch in range(args.n_epochs):
        ...
        train_loss = 0
        model1.train()
        for batch in tqdm(train_loader1, total=len(train_loader1)):
            model1.zero_grad()  
            input_ids = batch[0].to(args.device)
            input_mask = batch[1].to(args.device)
            labels = batch[2].float().to(args.device)
        ...
        valid_loss, valid_r2 = evaluate(model1, valid_loader1,loss_function)

        train_losses.append(train_loss)
        valid_losses.append(valid_loss)

        train_loss_avg = train_loss * args.batch_size / len(train_loader1)
        valid_loss_avg = valid_loss * args.batch_size / len(valid_loader1)
    ...
    for epoch in range(args.n_epochs):
        ...
        train_loss = 0
        model2.train()
        for batch in tqdm(train_loader2, total=len(train_loader2)):
            model2.zero_grad()  
            input_ids = batch[0].to(args.device)
            input_mask = batch[1].to(args.device)
            labels = batch[2].float().to(args.device)
        ...
        # Evaluation
        valid_loss, valid_r2 = evaluate(model2, valid_loader2,loss_function)

        train_losses.append(train_loss)
        valid_losses.append(valid_loss)

        train_loss_avg = train_loss * args.batch_size / len(train_loader2)
        valid_loss_avg = valid_loss * args.batch_size / len(valid_loader2)
